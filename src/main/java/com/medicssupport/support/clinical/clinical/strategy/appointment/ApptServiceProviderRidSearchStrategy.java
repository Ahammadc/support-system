...
@Component(AppointmentConstants.STRATEGY_APPT_SERVICE_PROVIDER_RID)
@Slf4j
public class ApptServiceProviderRidSearchStrategy extends SearchStrategyExtended<Appointment> {
    ...
    public Page<Appointment> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> serviceProviderRids = dataMap.getOrDefault(AppointmentConstants.KEY_APPT_SERVICE_PROVIDER_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching appointment by appt_service_provider_rid: {}", serviceProviderRids);

        return appointmentRepository.findAllByApptServiceProviderRidIn(serviceProviderRids, pageable);
    }
