...
@Component(UEntityConstants.STRATEGY_ENT_CODE)
@Slf4j
public class UEntityEntCodeSearchStrategy extends SearchStrategyExtended<UEntity> {
    ...
    public Page<UEntity> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<String> entCodes = dataMap.getOrDefault(UEntityConstants.KEY_ENT_CODE, List.of());

        log.info("Searching entity by ent_code: {}", entCodes);

        return uEntityRepository.findAllByEntCodeIn(entCodes, pageable);
    }
