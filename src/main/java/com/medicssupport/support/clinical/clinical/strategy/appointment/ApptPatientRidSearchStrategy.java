...
@Component(AppointmentConstants.STRATEGY_APPT_PATIENT_RID)
@Slf4j
public class ApptPatientRidSearchStrategy extends SearchStrategyExtended<Appointment> {
    ...
    public Page<Appointment> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> apptPatientRids = dataMap.getOrDefault(AppointmentConstants.KEY_APPT_PATIENT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching appointment by appt_patient_rid: {}", apptPatientRids);

        return appointmentRepository.findAllByApptPatientRidIn(apptPatientRids, pageable);
    }
