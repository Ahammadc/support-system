...
@Component(ApptResourceMappingConstants.STRATEGY_ARM_RID)
@Slf4j
public class ApptResourceMappingRidSearchStrategy extends SearchStrategyExtended<ApptResourceMapping> {
    ...
    public Page<ApptResourceMapping> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> armRids = dataMap.getOrDefault(ApptResourceMappingConstants.KEY_ARM_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching appointment resource mapping by arm_rid: {}", armRids);

        return apptResourceMappingRepository.findAllByArmRidIn(armRids, pageable);
    }
