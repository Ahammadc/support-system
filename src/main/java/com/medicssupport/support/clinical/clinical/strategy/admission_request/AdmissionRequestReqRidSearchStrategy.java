...
@Component(AdmissionRequestConstants.STRATEGY_REQ_RID)
@Slf4j
public class AdmissionRequestReqRidSearchStrategy extends SearchStrategyExtended<AdmissionRequest> {
    ...
    public Page<AdmissionRequest> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> reqRids = dataMap.getOrDefault(AdmissionRequestConstants.KEY_REQ_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching admission request by req_rid: {}", reqRids);

        return admissionRequestRepository.findAllByReqRidIn(reqRids, pageable);
    }
