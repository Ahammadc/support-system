...
@Component(AppointmentConstants.STRATEGY_APPT_UNIT_RID)
@Slf4j
public class ApptUnitRidSearchStrategy extends SearchStrategyExtended<Appointment> {
    ...
    public Page<Appointment> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> apptUnitRids = dataMap.getOrDefault(AppointmentConstants.KEY_APPT_UNIT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching appointment by appt_unit_rid: {}", apptUnitRids);

        return appointmentRepository.findAllByApptUnitRidIn(apptUnitRids, pageable);
    }
