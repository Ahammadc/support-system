...
@Component(UEntityConstants.STRATEGY_ENT_PARENT_RID_AND_ENT_TYPE)
@Slf4j
public class UEntityEntParentRidAndEntTypeSearchStrategy extends SearchStrategyExtended<UEntity> {
    ...
    public Page<UEntity> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> entParentRids = dataMap.getOrDefault(UEntityConstants.KEY_ENT_PARENT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        List<Integer> entTypes = dataMap.getOrDefault(UEntityConstants.KEY_ENT_TYPE, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching entity by ent_parent_rid: {} and ent_type: {}", entParentRids, entTypes);

        return uEntityRepository.findAllByEntParentRidInAndEntTypeIn(entParentRids, entTypes, pageable);
    }
