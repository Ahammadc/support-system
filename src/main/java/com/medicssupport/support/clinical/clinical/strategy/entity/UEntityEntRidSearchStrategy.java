...
@Component(UEntityConstants.STRATEGY_ENT_RID)
@Slf4j
public class UEntityEntRidSearchStrategy extends SearchStrategyExtended<UEntity> {
    ...
    public Page<UEntity> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> entRids = dataMap.getOrDefault(UEntityConstants.KEY_ENT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching entity by ent_rid: {}", entRids);

        return uEntityRepository.findAllByEntRidIn(entRids, pageable);
    }
