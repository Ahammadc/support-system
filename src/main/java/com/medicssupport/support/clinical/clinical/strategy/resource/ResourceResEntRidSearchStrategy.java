...
@Component(ResourceConstants.STRATEGY_RES_ENT_RID)
@Slf4j
public class ResourceResEntRidSearchStrategy extends SearchStrategyExtended<Resource> {
    ...
    public Page<Resource> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> resEntRids = dataMap.getOrDefault(ResourceConstants.KEY_RES_ENT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching resource by res_ent_rid: {}", resEntRids);

        return resourceRepository.findAllByResEntRidIn(resEntRids, pageable);
    }
