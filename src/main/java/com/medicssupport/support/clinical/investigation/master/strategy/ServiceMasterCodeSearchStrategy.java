...
@Component(ServiceMasterConstants.STRATEGY_SER_CODE)
@Slf4j
public class ServiceMasterCodeSearchStrategy extends SearchStrategyExtended<ServiceMaster> {
    ...
    public Page<ServiceMaster> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<String> codes = dataMap.getOrDefault(ServiceMasterConstants.KEY_SER_CODE, List.of());

        log.info("Searching service master by code: {}", codes);

        return serviceMasterRepository.findAllByCodeContainingIgnoreCase(codes.get(0), pageable);
    }
