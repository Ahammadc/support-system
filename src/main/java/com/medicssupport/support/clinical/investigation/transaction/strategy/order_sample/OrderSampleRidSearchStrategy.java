...
@Component(OrderSampleConstants.STRATEGY_SM_RID)
@Slf4j
public class OrderSampleRidSearchStrategy extends SearchStrategyExtended<OrderSample> {

    @Autowired
    private OrderSampleRepository repository;
    ...
    public Page<OrderSample> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> smRids = dataMap.getOrDefault(OrderSampleConstants.KEY_SM_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching order sample by sm_rid: {}", smRids);

        return repository.findAllBySmRidIn(smRids, pageable);
    }
