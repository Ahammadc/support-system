...
@Component(LabTestParamsConstants.STRATEGY_LTP_CATEGORY_RID)
@Slf4j
public class LabTestParamsCategoryRidSearchStrategy extends SearchStrategyExtended<LabTestParams> {
    ...
    public Page<LabTestParams> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> categoryRids = dataMap.getOrDefault(LabTestParamsConstants.KEY_LTP_CATEGORY_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching lab test params by category_rid: {}", categoryRids);

        return labTestParamsRepository.findAllByCategoryRidIn(categoryRids, pageable);
    }
