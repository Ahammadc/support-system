...
@Component(ResultObservationConstants.STRATEGY_RO_RD_RID)
@Slf4j
public class ResultObservationRdRidSearchStrategy extends SearchStrategyExtended<ResultObservation> {
    ...
    public Page<ResultObservation> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> roRdRids = dataMap.getOrDefault(ResultObservationConstants.KEY_RO_RD_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching result observation by ro_rd_rid: {}", roRdRids);

        return repository.findAllByRoRdRidIn(roRdRids, pageable);
    }
