...
@Component(NormalRangeConstants.STRATEGY_NR_AC_RID)
@Slf4j
public class NormalRangeAcRidSearchStrategy extends SearchStrategyExtended<NormalRange> {
    ...
    public Page<NormalRange> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> acRids = dataMap.getOrDefault(NormalRangeConstants.KEY_NR_AC_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching normal range by ac_rid: {}", acRids);

        return normalRangeRepository.findAllByAcRidIn(acRids, pageable);
    }
