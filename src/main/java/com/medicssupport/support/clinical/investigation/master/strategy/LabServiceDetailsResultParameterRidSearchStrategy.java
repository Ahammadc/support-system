...
@Component(LabServiceDetailsConstants.STRATEGY_LSD_RESULT_PARAMETER_RID)
@Slf4j
public class LabServiceDetailsResultParameterRidSearchStrategy extends SearchStrategyExtended<LabServiceDetails> {
    ...
    public Page<LabServiceDetails> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> resultParameterRids = dataMap.getOrDefault(LabServiceDetailsConstants.KEY_LSD_RESULT_PARAMETER_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching lab service details by result_parameter_rid: {}", resultParameterRids);

        return labServiceDetailsRepository.findAllByResultParameterRidIn(resultParameterRids, pageable);
    }
