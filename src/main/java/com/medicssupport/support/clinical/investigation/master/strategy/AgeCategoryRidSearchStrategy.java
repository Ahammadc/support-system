...
@Component(AgeCategoryConstants.STRATEGY_AC_RID)
@Slf4j
public class AgeCategoryRidSearchStrategy extends SearchStrategyExtended<AgeCategory> {
    ...
    public Page<AgeCategory> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> rids = dataMap.getOrDefault(AgeCategoryConstants.KEY_AC_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching age category by rid: {}", rids);

        return ageCategoryRepository.findAllByRidIn(rids, pageable);
    }
