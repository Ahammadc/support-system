...
@Component(ServiceOrderConstants.STRATEGY_SO_RID)
@Slf4j
public class ServiceOrderRidSearchStrategy extends SearchStrategyExtended<ServiceOrder> {
    ...
    public Page<ServiceOrder> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> soRids = dataMap.getOrDefault(ServiceOrderConstants.KEY_SO_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching service order by so_rid: {}", soRids);

        return repository.findAllBySoRidIn(soRids, pageable);
    }
