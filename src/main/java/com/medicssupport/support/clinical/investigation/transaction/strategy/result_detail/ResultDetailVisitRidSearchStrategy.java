...
@Component(ResultDetailConstants.STRATEGY_RD_VISIT_RID)
@Slf4j
public class ResultDetailVisitRidSearchStrategy extends SearchStrategyExtended<ResultDetail> {
    ...
    public Page<ResultDetail> search(List<SearchData> searchData, Pageable pageable) {
        ...
        List<Integer> rdVisitRids = dataMap.getOrDefault(ResultDetailConstants.KEY_RD_VISIT_RID, List.of()).stream()
                .filter(val -> val.matches(CommonConstants.NUMERIC_REGEX))
                .map(Integer::valueOf)
                .toList();

        log.info("Searching result detail by rd_visit_rid: {}", rdVisitRids);

        return repository.findAllByRdVisitRidIn(rdVisitRids, pageable);
    }
